...
public class Arm
{
    ...
    public void draw()
    {
        ...
        // distance inbetween
        // xj2 - xj1 = xj// dis x wise
        // Math.abs(yj2 - yj1) = yj// dist y wise
        // math.Sqrt(xj^2 + yj^2) // dist total between jpoints
    ...
    public void directKinematic(){
        ...
        xj = xj2 - xj1;// dis x wise
        yj = Math.abs(yj2 - yj1);// dist y wise
        xa = xj *.5; // half the x difference 
        ya = yj * .5;
        double dInbetweenJoints = Math.sqrt((xj*xj) + (yj*yj)); // dist total between jpoints
        //(r * Math.cos(theta1) - r * Math.cos(theta2)) + xm2 - xm1;//...;
        /*#...*/
        if (dInbetweenJoints<2*r){
            valid_state = true;
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double dx1 = xt - xm1; 
        double dy1 = yt - ym1;
        // distance between pem/pen? and motor
        double d1 = null /*#...*/;
        if (d1>2*r){
